...
export default function SupportButton() {
  ...
  const [messages, setMessages] = useState([
    ...
    {
      type: "options",
      options: FAQ_TREE.map((f) => ({ id: f.id, label: f.question })),
    },
  ]);
  ...
  const handleOptionClick = (optionId) => {
    let faqItem = FAQ_TREE.find((f) => f.id === optionId);

    if (!faqItem) {
      for (const top of FAQ_TREE) {
        const sub = (top.followups || []).find((f) => f.id === optionId);
        if (sub) { faqItem = sub; break; }
      }
    }
    ...
    if (faqItem.followups && faqItem.followups.length > 0) {
      newMessages.push({
        type: "options",
        options: faqItem.followups.map((f) => ({ id: f.id, label: f.question })),
      ...
      newMessages.push({
        type: "options",
        options: [
          { id: "__back", label: "← Back to all topics" },
          { id: "__done", label: "Thanks, that helped!" },
        ],
        prefix: "Was that helpful?",
      });
    }
  ...
  const handleOptionSelect = (id) => {
    if (id.startsWith("__")) handleSpecialOption(id);
    else handleOptionClick(id);
  };
  ...
  const handleInputSend = () => {
    ...
    let matchedFaq = null;
    const allFaqs = FAQ_TREE.reduce(
      (acc, top) => [...acc, top, ...(top.followups || [])],
      []
    );

    for (const faq of allFaqs) {
      const keywords = faq.question.toLowerCase().split(/\W+/);
      const inputWords = lower.split(/\W+/);
      const overlap = keywords.filter((w) => w.length > 3 && inputWords.includes(w));
      if (overlap.length >= 1) { matchedFaq = faq; break; }
    }

    const newMessages = [...messages, { type: "user", text }];

    if (matchedFaq) {
      newMessages.push({ type: "bot", text: matchedFaq.answer });
      if (matchedFaq.followups?.length) {
        newMessages.push({
          type: "options",
          options: matchedFaq.followups.map((f) => ({ id: f.id, label: f.question })),
          prefix: "Related questions:",
        });
      } else {
        newMessages.push({
          type: "options",
          options: [{ id: "__back", label: "← Back to all topics" }],
        });
      }
    } else {
      newMessages.push({
